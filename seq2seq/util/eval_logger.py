...
class EvalLogger(object):
    ...
    def check_decode_eval(self):
        ...
        for i in range(input_size):
            ...
            log_line = ("sample %s - input: %s" % (i+1, decode_input)).encode('utf-8')
            self.log_writer.write("%s\r\n"%(log_line.decode('utf-8')))
            print(log_line)
            decode_output = self.decode_sample_output[i]
            log_line = ("sample %s - output: %s" % (i+1, decode_output)).encode('utf-8')
            self.log_writer.write("%s\r\n" % (log_line.decode('utf-8')))
            print(log_line)
            decode_reference = self.decode_sample_reference[i]
            log_line = ("sample %s - reference: %s" % (i+1, decode_reference)).encode('utf-8')
            self.log_writer.write("%s\r\n" % (log_line.decode('utf-8')))
            print(log_line)
